...
const gatherRelatedFiles = (file, namedObj) => {
	...
	while (queue.length) {
		...
		importing.forEach((name) => {
			const fileSrc = namedObj[name];
			files.push(fileSrc);
			queue.push(fileSrc.content);
		});
	}
	...
	md.core.ruler.push('add-vue-demos', (state) => {
		...
		for (let i = 0; i <  tokens.length; i++) {
			...
			if (name) {
				...
				const token = Object.assign(new state.Token('html_block', '', 0), { content });
				replaceWith.push(token);
				output.snippets.entry.push({ content, token });
			}
		...
		output.snippets.entry.forEach((file, idx) => {
			...
			relatedFiles.forEach(file => {
				file.content = rewriteImports(opts.resourcePath, file.content);
			});
